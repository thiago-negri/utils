...
#            your new version of the file (moved, but without the remote changes). so if
#            you simply delete the file in your working tree, you will lose the change that
...
#            not be an issue.
alias sync      "!f() { if test \$(git rv | wc -l) -eq 0; then return; fi; local dirty=\$(git status -s | wc -l); local before=\$(echo '### BEFORE'; git l -n 5 --color=always --decorate=short; git s); if test \$dirty -ne 0; then git stash -q; fi; local p=\$(git pullr 2>/dev/null); git push -q; if test \$dirty -ne 0; then git stash apply -q; git rr -q; git stash drop -q; fi; if test \"\$p\" != \"Already up to date.\"; then echo ''; echo \"\$before\"; echo ''; echo '### AFTER'; git l -n 5; git s; echo ''; fi; }; f"
